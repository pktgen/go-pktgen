...
#ifndef __GAPI_MUTEX_HELPER_H
#define __GAPI_MUTEX_HELPER_H
 ...
 */
static inline int
gapi_mutex_create(pthread_mutex_t *mutex, int flags)
{
...
err:
    if (inited) {
        ...
        if (pthread_mutexattr_destroy(&attr))
            printf("Failed to destroy mutex attribute, but is not the root cause\n");
    }
 ...
 */
static inline int
gapi_mutex_destroy(pthread_mutex_t *mutex)
{
    int ret = 0;
