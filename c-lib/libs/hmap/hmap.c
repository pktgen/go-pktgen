...
hmap_create(const char *name, uint32_t max_capacity, hmap_funcs_t *funcs)
{
    ...
    if (gapi_mutex_create(&hmap->mutex, PTHREAD_MUTEX_RECURSIVE) < 0)
...
hmap_destroy(hmap_t *hmap)
{
    ...
    if (gapi_mutex_destroy(&hmap->mutex))
...
RTE_INIT_PRIO(hmap_constructor, LOG)
{
    ...
    if (gapi_mutex_create(&hmap_list_mutex, PTHREAD_MUTEX_RECURSIVE) < 0)
