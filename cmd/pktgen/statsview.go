...
func (sv *StatsView) DisplayStats() {
	...
	for v := uint16(0); v < sv.portCnt; v++ {
		...
		runtJumbos := fmt.Sprintf("%s/%s", comma(types.RuntPackets), comma(types.JumboPackets))
		arpIcmps := fmt.Sprintf("%s/%s", comma(types.ArpPackets), comma(types.IcmpPackets))
		...
		rowData := []hlp.TextInfo{
			...
			hlp.NewText(cz.Wheat(types.Broadcast), kview.AlignRight),            // Broadcast packets
			hlp.NewText(cz.GoldenRod(types.Multicast), kview.AlignRight),        // Multicast packets
			hlp.NewText(cz.Cyan(types.Size64), kview.AlignRight),                // 64-byte packets
			hlp.NewText(cz.Cyan(types.Size65To127), kview.AlignRight),           // 65-127-byte packets
			hlp.NewText(cz.Cyan(types.Size128To255), kview.AlignRight),          // 128-255-byte packets
			hlp.NewText(cz.Cyan(types.Size256To511), kview.AlignRight),          // 256-511-byte packets
			hlp.NewText(cz.Cyan(types.Size512To1023), kview.AlignRight),         // 512-1023-byte packets
			hlp.NewText(cz.Cyan(types.Size1024To1518), kview.AlignRight),        // 1024-1518-byte packets
