...
// Introduction returns a kview.List with the highlights of the kview package.
func Introduction(nextSlide func()) (title string, info string, content kview.Primitive) {
	list := kview.NewList()
	...
	reset := func() {
		...
		for i, itemText := range listText {
			item := kview.NewListItem(itemText[0])
