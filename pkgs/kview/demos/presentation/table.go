...
const tableBasic = `[green]func[white] [yellow]main[white]() {
    table := kview.[yellow]NewTable[white]().
...
func Table(nextSlide func()) (title string, info string, content kview.Primitive) {
	table := kview.NewTable()
	table.SetFixed(1, 1)
	for row, line := range strings.Split(tableData, "\n") {
		for column, cell := range strings.Split(line, "|") {
			color := kview.Styles.PrimaryTextColor
			if row == 0 {
				color = kview.Styles.SecondaryTextColor
			} else if column == 0 {
				color = kview.Styles.TertiaryTextColor
			}
			align := kview.AlignLeft
			if row == 0 {
				align = kview.AlignCenter
			...
			tableCell := kview.NewTableCell(cell)
	...
	separator := func() {
		table.SetBorders(false)
		table.SetSelectable(false, false)
		table.SetSeparator(kview.Borders.Vertical)
		code.Clear()
	...
	for _, tableText := range demoTableText {
		item := kview.NewListItem(tableText.text)
