...
[green]import[white] [red]"github.com/pktgen/go-pktgen/pkgs/kview"[white]

[green]func[white] [yellow]main[white]() {
	$$$

	root := kview.[yellow]NewTreeNode[white]([red]"Root"[white]).
		[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"First child"[white]).
			[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Grandchild A"[white])).
			[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Grandchild B"[white]))).
		[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Second child"[white]).
			[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Grandchild C"[white])).
			[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Grandchild D"[white]))).
		[yellow]AddChild[white](kview.[yellow]NewTreeNode[white]([red]"Third child"[white]))
...
const treeBasicCode = `tree := kview.[yellow]NewTreeView[white]()`

const treeTopLevelCode = `tree := kview.[yellow]NewTreeView[white]().
		[yellow]SetTopLevel[white]([red]1[white])`

const treeAlignCode = `tree := kview.[yellow]NewTreeView[white]().
		[yellow]SetAlign[white](true)`

const treePrefixCode = `tree := kview.[yellow]NewTreeView[white]().
...
func TreeView(nextSlide func()) (title string, info string, content kview.Primitive) {
	treeNextSlide = nextSlide
	tree.SetBorder(true)
	tree.SetTitle("TreeView")

	// Add nodes.
	var add func(target *node) *kview.TreeNode
	add = func(target *node) *kview.TreeNode {
		node := kview.NewTreeNode(target.text)
	...
	tree.SetSelectedFunc(func(n *kview.TreeNode) {
