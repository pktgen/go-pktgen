...
func main() {
	...
	prefixMap := make(map[string][]*kview.ListItem)
	inputField.SetAutocompleteFunc(func(currentText string) []*kview.ListItem {
		// Ignore empty text.
		...
		go func() {
			...
			entries := make([]*kview.ListItem, 0, len(companies))
			for _, c := range companies {
				entries = append(entries, kview.NewListItem(c.Name))
			}
			mutex.Lock()
