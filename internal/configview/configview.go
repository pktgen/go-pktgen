...
func Create(panels *kview.Panels, to *tab.Tab, portCnt int, tabChar string, flex *kview.Flex) *ConfigView {
...
func (cv *ConfigView) ConfigForm(port int) *kview.Flex {
...
func (cv *ConfigView) initConfigView(tabChar string) {
...
func (cv *ConfigView) setupEditConfigForm(port int, tabChar string) *kview.Flex {
...
func (cv *ConfigView) DisplayConfigTable() {
	...
	for v := 0; v < cv.portCnt; v++ {
		...
		rowData := []hlp.TextInfo{
			hlp.NewText(state(cfg.PortIndex, cfg.TxState), kview.AlignLeft),
