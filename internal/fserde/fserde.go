...
func init() {
	dbug = dbg.New()
	/*
		dbug.AddFiles([]string{"tcp.go", "ipv4.go"})
		dbug.AddFunctions([]string{
			"(*PayloadLayer).Parse",
			"(*Frame).toBinaryUpdateL4Checksum",
			"(*MyBuffer).WriteValueAt",
			"(*UDPLayer).ApplyDefaults",
			"(*IPv4Layer).Parse",
		})
		dbug.SetPrintState(true)
	*/
}
...
func (fr *Frame) GetOffset(name LayerName) uint16 {
	...
	dbug.Printf("LayerName %s\n", name)
	for _, proto := range fr.protocols {
		if proto.name == name { // Stop at the given layer
			break
		}
		dbug.Printf("Getting for %-12s - offset %3d len %4d\n",
			proto.name, proto.offset, proto.length)
		offset += proto.length
	}
	dbug.Printf("offset %3d\n", offset)
	return offset
}
