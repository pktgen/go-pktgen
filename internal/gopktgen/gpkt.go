...
func createGoPktgen() *GoPktgen {

	// Create a new GoPktgen instance with default values
	gPkt := &GoPktgen{
		GoPktgenApi: GoPktgenApi{},
		l2p: &gpc.L2p{
			Cores:   make(map[gpc.CoreID]*gpc.LogicalCore),
			LPorts:  make(map[gpc.LPortID]*gpc.LogicalPort),
			Ports:   make(map[gpc.PortID]*gpc.PhysicalPort),
			MapList: []*gpc.Mapping{},
		},
		libList:   []*LibInfo{},
...
func (g *GoPktgen) Start() error {
	...
	for i := uint16(0); i < g.PortCount(); i++ {
		...
		tlog.Printf("Port %d: RxQueues: %d, TxQueues: %d\n", i, numRx, numTx)
