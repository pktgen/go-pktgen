 ...
 * SEE ALSO:
 */
func (g *GoPktgen) processMap(m *gpc.Mapping) error {

	var lcore *gpc.LogicalCore
	var port *gpc.PhysicalPort
	...
	if ok {
        ...
        lcore = &gpc.LogicalCore{Core: m.Core, Mode: m.Mode.Value()}
	...
	if lcore.Mode != gpc.MainMode && lcore.Mode != gpc.UnknownMode {
		...
		if ok {
			...
			port = &gpc.PhysicalPort{Pid: gpc.PortID(m.Port)}

			l.Ports[m.Port] = port
			l.PortCount++
		}
	} else {
		port = &gpc.PhysicalPort{Pid: gpc.MaxEtherPorts}
	}
	...
	tlog.Printf("Created logical port: %s\n", gpc.MarshalIndent(lport))

	return nil
}

func (g *GoPktgen) newLogicalPort(port *gpc.PhysicalPort, mode gpc.CoreMode) *gpc.LogicalPort {
	var qid gpc.QueueID
